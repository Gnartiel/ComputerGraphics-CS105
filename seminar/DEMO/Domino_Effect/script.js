...
initScene = function () {
    ...
    renderer = THREE.WebGL1Renderer ? new THREE.WebGL1Renderer() : new THREE.WebGLRenderer;
    ...
    var controls = new function () {
        ...
        this.updateGravity = function () {
            scene.setGravity(new THREE.Vector3(controls.gravityX, controls.gravityY, controls.gravityZ));
            stones.forEach(function (st) {
                st.setLinearVelocity(new THREE.Vector3()); // Đặt vận tốc tuyến tính về 0 để đảm bảo đối tượng không tiếp tục chuyển động từ khung hình trước
            });
        };
    };
    ...
    controls.updateGravity()
        ...
        function createGround() {
            var ground_material = Physijs.createMaterial(
                    new THREE.MeshPhongMaterial({map: THREE.ImageUtils.loadTexture('../assets/textures/floor-wood.jpg')}),
                    .9, .3);
