...
/*
	Xây dựng cấu trúc STK - Dùng trong tô màu theo đường biên
*/
STK::STK(int size)
{
	n = 0;
...
void STK::Pop(int& xc, int& yc)
{
	--n; //Chưa kiểm tra STK rỗng hay chưa
	xc = x[n];
	yc = y[n];
}
...
void CPolygon::Draw(CDC* pDC, COLORREF color)
{
	//pDC->MoveTo(p[n - 1]);
	
	for (int i = 0; i < n; ++i)
	{
		pDC->SetPixel(p[i], color);
	}
...
//Cải tiến thuật toán gốc - Khử đệ quy bằng STK
...
//Cải tiến thuật toán khử đệ quy bằng STK - Tô không phụ thuộc màu biên
